...
class IonizationChamber:
    ...
    @staticmethod
    def get_time(time_readings):
        return series_to_magnitude(series=time_readings, unit='s')

    @staticmethod
    def get_charge(charge_readings):
        return series_to_magnitude(series=charge_readings, unit='C')

    @staticmethod
    def get_temperature(temperature_readings):
        return series_to_magnitude(series=temperature_readings, unit='ºC')

    @staticmethod
    def get_pressure(pressure_readings):
        return series_to_magnitude(series=pressure_readings, unit='kPa')

    def get_current(self, charge_readings, time_readings, open_shutter, leak_current_value=None,
                    temperature_readings=None, pressure_readings=None):
        current = self.measure_current_readings(charge_readings=charge_readings, time_readings=time_readings,
                                                open_shutter=open_shutter, leak_current_value=leak_current_value,
                                                temperature_readings=temperature_readings,
                                                pressure_readings=pressure_readings)
        return series_to_magnitude(series=current, unit='A')

    def get_air_kerma(self, current_readings, radiation_quality):
        air_kerma = self.measure_air_kerma_readings(current_readings=current_readings,
                                                    radiation_quality=radiation_quality)
        return series_to_magnitude(series=air_kerma, unit='Gy/s')

    def measure_leakage(self, leakage_charge_readings, leakage_time_readings):
        # Leakage current measurements (closed shutter)
        leakage_current_readings = self.measure_current_readings(
            charge_readings=leakage_charge_readings, time_readings=leakage_time_readings, open_shutter=False)
        leakage_time = self.get_time(time_readings=leakage_time_readings)
        leakage_charge = self.get_charge(charge_readings=leakage_charge_readings)
        leakage_current = self.get_current(time_readings=leakage_time_readings, charge_readings=leakage_charge_readings,
                                           open_shutter=False)

        # Define lists to build dataframe
        magnitudes = [leakage_time, leakage_charge, leakage_current]
        names = ['Leakage time', 'Leakage charge', 'Leakage current']
        readings = [leakage_time_readings, leakage_charge_readings, leakage_current_readings]
    ...
    def measure_current(self, charge_readings, time_readings, temperature_readings, pressure_readings, leakage_current_value):

        # Total current measurements (open shutter)
        current_readings = self.measure_current_readings(
            ...
            leak_current_value=leakage_current_value, temperature_readings=temperature_readings,
            pressure_readings=pressure_readings)
        time = self.get_time(time_readings=time_readings)
        pressure = self.get_pressure(pressure_readings=pressure_readings)
        temperature = self.get_temperature(temperature_readings=temperature_readings)
        charge = self.get_charge(charge_readings=charge_readings)
    ...
    def measure_air_kerma(self, current_readings, radiation_quality):
        ...
        air_kerma_readings = self.measure_air_kerma_readings(current_readings=current_readings,
                                                             radiation_quality=radiation_quality)
        air_kerma = self.get_air_kerma(current_readings=current_readings, radiation_quality=radiation_quality)
    ...
    def measure(self, leakage, current, air_kerma, leakage_time_readings=None, leakage_charge_readings=None,
        ...
        if leakage:
            # Leakage current measurements (closed shutter)
            leakage = self.measure_leakage(leakage_charge_readings=leakage_charge_readings,
                                           leakage_time_readings=leakage_time_readings)
            measurement = leakage
            if current:
                # Total current measurements (open shutter)
                ...
                current = self.measure_current(
                    ...
                    pressure_readings=pressure_readings, leakage_current_value=leakage_current_value)
