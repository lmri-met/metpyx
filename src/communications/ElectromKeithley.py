...
class ElectrometroGUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.widget_frame, text="Tipo de medida:").grid(row=3, column=0)
        self.tipo_medida_optionmenu = tk.OptionMenu(self.widget_frame, self.tipo_medida_var, "Fondo", "Nucleido")
        self.tipo_medida_optionmenu.grid(row=3, column=1)

        # Campo para introducir el número de serie de medidas
        tk.Label(self.widget_frame, text="Número de serie de medidas:").grid(row=4, column=0)
        self.num_serie_entry = tk.Entry(self.widget_frame, textvariable=self.num_serie_var)
        self.num_serie_entry.grid(row=4, column=1)

        # Campo para introducir el tiempo entre series
        tk.Label(self.widget_frame, text="Tiempo entre series (segundos):").grid(row=5, column=0)
        self.tiempo_entre_medidas_entry = tk.Entry(self.widget_frame, textvariable=self.tiempo_entre_medidas_var)
        self.tiempo_entre_medidas_entry.grid(row=5, column=1)

        # Campo para introducir el número de medidas
        tk.Label(self.widget_frame, text="Número de medidas:").grid(row=6, column=0)
    ...
    def update_graph(self):
        ...
        for tanda in self.tandas:
            ...
            self.ax.plot(datos, tanda['datos'], marker='o', linestyle='-', color=tanda['color'], label=f'{tanda["tipo_medida"]} (Histórico)')
    ...
    def save_to_excel(self):
        ...
        # Leer los datos del archivo 'datos.txt' usando pandas
        try:
            df = pd.read_csv('datos.txt', sep='\s+', decimal=',', header=0, encoding='ISO-8859-1')
        except Exception as e:
            messagebox.showerror("Error", "No se pudo leer el archivo de datos: " + str(e))
            return
        ...
        ws['A4'] = "Tipo de medida:"
        ws['B4'] = self.tipo_medida_var.get()
        ...
        for row_idx, item in enumerate(self.table.get_children(), start=13):
        ...
        # Usar xlsxwriter como motor de ExcelWriter
        with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
            ...
            formato_numerico = workbook.add_format({'num_format': '0.00'})
            formato_exponencial = workbook.add_format({'num_format': '0.00000E+00', 'decimal_separator': ','})
            formato_texto = workbook.add_format({'num_format': '@'})

            # Aplicar los formatos
            worksheet.set_column('A:B', None, formato_numerico)
            worksheet.set_column('C:D', None, formato_exponencial)
            worksheet.set_column('E:F', None, formato_texto)
        ...
        for col_index, encabezado in enumerate(encabezados, start=13):  
        ...
        try:
            with open('datos.txt', 'r') as f:
                ...
                for index, line in enumerate(lines[1:], start=22):  # Ignora el encabezado en el archivo de datos
                    for col_index, value in enumerate(line.strip().split(), start=13):  # Asumiendo espacio como delimitador
