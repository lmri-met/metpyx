...
class ElectrometroGUI:
    def __init__(self, master):
        ...
        self.master = master
        self.master.title("Cámara IG-11 - Sistema Internacional de Referencia - Emisores Gamma")
        self.master.configure(bg='#fee4b9') 
        self.table = None
        ...
        self.sigma_medio = 0
        self.factor = 0 
        self.valor_electrom = 0
        self.ultima_medida_directa = 0 
        self.num_serie_actual = 1
        self.start_time = None
        self.running = False  # Variable para controlar si se está tomando datos

        self.nucleido_var = tk.StringVar(value='Elige Nucleido')  # Para seleccionar el Nucleido
        self.contenedor_var = tk.StringVar(value='Elige Contenedor')  # Para seleccionar el Contenedor basado en el Nucleido seleccionado
        self.df = pd.DataFrame()  # DataFrame vacío, se llenará con los datos del Excel
    ...
    def create_widgets(self):
        ...
        tk.Label(self.widget_frame, text="Nombre de la Medida:", bg='#fee4b9').grid(row=0, column=0)
        ...
        self.nombre_medida_entry.configure(bg='white') 

        # Campo para introducir la persona que realiza la medida
        tk.Label(self.widget_frame, text="Persona que realiza la medida:", bg='#fee4b9').grid(row=1, column=0)
        ...
        self.persona_medida_entry.configure(bg='white') 

        # Campo para mostrar la fecha y hora de la medida
        tk.Label(self.widget_frame, text="Fecha y Hora de la medida:", bg='#fee4b9').grid(row=2, column=0)
        ...
        self.fecha_hora_medida_label.configure(bg='white') 
        
        # Campo para seleccionar el puerto COM
        tk.Label(self.widget_frame, text="Puerto COM:", bg='#fee4b9').grid(row=3, column=0)  # Nota el cambio de índice de fila a 3
        ...
        self.puerto_com_optionmenu.configure(bg='white')
        ...
        tk.Label(self.widget_frame, text="Elige Nucleido o Fondo:", bg='#fee4b9').grid(row=4, column=0)
        self.nucleido_optionmenu = tk.OptionMenu(self.widget_frame, self.nucleido_var, 'Elige Nucleido o Fondo')
        self.nucleido_optionmenu.grid(row=4, column=1)
        self.nucleido_optionmenu.configure(bg='white')

        tk.Label(self.widget_frame, text="Contenedor:", bg='#fee4b9').grid(row=5, column=0)
        self.contenedor_optionmenu = tk.OptionMenu(self.widget_frame, self.contenedor_var, 'Elige Contenedor')
        self.contenedor_optionmenu.grid(row=5, column=1)
        self.contenedor_optionmenu.configure(bg='white')

        # Botón para mostrar el resultado
        # tk.Button(self.widget_frame, text="Mostrar Resultado", command=self.mostrar_resultado).grid(row=2, column=0, columnspan=2)

        # Campo para introducir el número de medidas
        tk.Label(self.widget_frame, text="Número de medidas:", bg='#fee4b9').grid(row=6, column=0)
        self.num_medidas_entry = tk.Entry(self.widget_frame, textvariable=self.num_medidas_var)
        self.num_medidas_entry.grid(row=6, column=1)
        self.num_medidas_entry.configure(bg='#fee4b9')
    ...
    def load_data_from_excel(self):
        print("Entra en load data excel")
        archivo_excel = 'Factores2.xlsx'
        self.df = pd.read_excel(archivo_excel, engine='openpyxl')

        nucleidos = self.df['Nucleido'].unique()
        menu = self.nucleido_optionmenu["menu"]
        menu.delete(0, "end")
        for nucleido in nucleidos:
            menu.add_command(label=nucleido, command=lambda value=nucleido: self.nucleido_var.set(value))

        # Actualizar el menú de Contenedor cuando se selecciona un Nucleido
        self.nucleido_var.trace('w', self.actualizar_contenedores)
        self.contenedor_var.trace('w', self.mostrar_resultado)

    def actualizar_contenedores(self, *args):
        print("Entra en contenedores")
        nucleido_seleccionado = self.nucleido_var.get()
        df_filtrado = self.df[self.df['Nucleido'] == nucleido_seleccionado]

        contenedores = df_filtrado['Contenedor'].unique()
        menu = self.contenedor_optionmenu["menu"]
        menu.delete(0, "end")
        for contenedor in contenedores:
            menu.add_command(label=contenedor, command=lambda value=contenedor: self.contenedor_var.set(value))

    def set_contenedor_and_show(self, value):
        self.contenedor_var.set(value)
        self.mostrar_resultado()

    def mostrar_resultado(self, *args, **kwargs):
        print("Entra en mostrar resultado")
        nucleido_seleccionado = self.nucleido_var.get()
        contenedor_seleccionado = self.contenedor_var.get()
        df_filtrado = self.df[(self.df['Nucleido'] == nucleido_seleccionado) & (self.df['Contenedor'] == contenedor_seleccionado)]
        print(f"df_filtrado: {df_filtrado}")
        if not df_filtrado.empty:
            factor = df_filtrado.iloc[0]['factor (Bq/pA)']
            messagebox.showinfo("Resultado", f"Nucleido: {nucleido_seleccionado}\nContenedor: {contenedor_seleccionado}\nFactor (Bq/pA): {factor}")
            self.factor = df_filtrado.iloc[0]['factor (Bq/pA)']  # Ahora guardando el factor como una variable de instancia
            print(f"coeficiente mostrar resultado: {self.factor}")

        else:
            messagebox.showerror("Error", "No se encontró el factor para la combinación seleccionada.")
    ...
    def start_acquisition(self):
        ...
        if self.todos_valores_medidos:
            self.tandas.append({
                ...
                'tipo_medida': self.nucleido_var.get()
            })
    ...
    def insert_last_series_data(self):
        ...
        self.table.insert("", "end", values=(self.num_serie_actual, f"{self.valor_medio:.4e}", f"{self.sigma_medio:.4e}",
                                        ...
                                        f"{time_diff_seconds:.1f}", self.nucleido_var.get()))  # Se añade el tipo de medida
        ...
        for serie in range(1, num_serie + 1):
            ...
            for medida in range(1, num_medidas + 1):
                ...
                if seleccion == "SIMULACIÓN":
                    ...
                    self.valor_electrom = valor_medido
                    if hasattr(self, 'factor'):
                        self.valor_medido = valor_medido * self.factor
                    else:
                        print("El factor no ha sido establecido.")
                else:
                ...
                print(f"Valor medido previo a multiplicación: {valor_medido}")
                print(f"Valor electrómetro: {self.valor_electrom}")
                self.valor_medido = valor_medido * self.factor
                valor_medido = valor_medido * self.factor
                ...
                print(f"Valor medido * factor: {self.valor_medido}")
                ...
                if len(self.valores_medidos) >= 2:
                    # Corregir el cálculo de sigma medio
                    self.sigma_medio = statistics.stdev([float(valor) for valor in self.valores_medidos]) / (len(self.valores_medidos) ** 0.5)
                ...
                self.escribir_datos(medida, serie, valor_medido, sigma=self.sigma_medio, tiempo=tiempo, tipo=self.nucleido_var.get())
            ...
            valor_medio_serie_actual = statistics.mean([float(val) for val in self.valores_medidos])
            sigma_medio_serie_actual = statistics.stdev([float(val) for val in self.valores_medidos]) if len(self.valores_medidos) > 1 else 0
    ...
    def show_warning_message(self):
        ...
        if valores_medios and sigmas_medios:
            ...
            self.table.insert("", "end", values=(
                ...
                f"{duracion_total_seconds:.1f}",  # Duración total en segundos
                self. nucleido_var.get(),
                ""
            ))
    ...
    def update_graph(self):
        ...
        if self.todos_valores_medidos:
            ...
            self.ax.plot(datos_actuales, self.todos_valores_medidos, marker='o', linestyle='-', color=color_actual, label=f'{self. nucleido_var.get()}')
            # self.ax.plot(datos_actuales, self.todos_valores_medidos, marker='o', linestyle='-', color=color_actual, label=f'{self.tipo_medida_var.get()} (Actual)')
            
            # Agregar etiquetas en cada punto para la tanda actual
            for i, valor in enumerate(self.todos_valores_medidos):
                etiqueta = f"{i+1}/{len(self.todos_valores_medidos)}\n"
    ...
    def format_results(self):
        ...
        if hasattr(self, 'ultima_medida_directa') and hasattr(self, 'factor'):
            self.valor_medido = self.ultima_medida_directa * self.factor
        else:
            self.valor_medido = 'No definido'  # o algún manejo de error
        factor_value = getattr(self, 'factor', 'No definido')  # Usa 'No definido' si 'factor' no existe
        result_str += f"Coeficiente: {factor_value}\n"
        valor_electrometro = getattr(self, 'ultima_medida_directa', 'Valor no definido')
        result_str += f"Valor del electrómetro: {self.valor_electrom}\n"
